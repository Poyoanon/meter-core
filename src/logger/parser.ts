    ...
    */
    ...
    this.#logger
      ...
      .on("BlockSkillStateNotify", (pkt) => {
        ...
        if (!parsed) return;
        console.log(
          pkt.time.toLocaleTimeString() +
            "###Stagger" +
            "|" +
            parsed.objectId +
            "|" +
            parsed.paralyzationMaxPoint +
            "|" +
      ...
      .on("IdentityGaugeChangeNotify", (pkt) => {
        ...
        console.log(
          "###Identity" +
            ...
            parsed.playerId +
            " | IdentityGauge1: " +
            parsed.identityGauge1 +
            " | IdentityGauge2: " +
            parsed.identityGauge2 +
            " | IdentityGauge3: " +
            parsed.identityGauge3
        );
      })
      .on("InitAbility", (pkt) => {})
      .on("InitEnv", (pkt) => {})
      ...
      .on("ParalyzationStateNotify", (pkt) => {
        const parsed = pkt.parsed;
        if (!parsed) return;
        //console.log("###StaggerBar" + "|" + parsed.ObjectId + "|" + parsed.HitCheckTime + "|" + parsed.Enable + "|" + parsed.DecreasePoint +"|" + parsed.ParalyzationPoint + "|" + parsed.ParalyzationMaxPoint);
        if (parsed.paralyzationPoint !== 0) {
